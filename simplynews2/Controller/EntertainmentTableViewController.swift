...
class EntertainmentTableViewController: UITableViewController , XMLParserDelegate {
    ...
    func loadData() {
        url = URL(string: "http://www.instyle.com/feeds/all/ins.rss")!
        loadRss(url);
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "openPagefive" {
            let indexPath: IndexPath = self.tableView.indexPathForSelectedRow!
            let selectedURL: String = (Feeds[indexPath.row] as AnyObject).object(forKey: "link") as! String
            
            
            let entertainmentinstance: EntertainmentViewController = segue.destination as! EntertainmentViewController
            entertainmentinstance.selectedcellurl = selectedURL as String
        }
    }
