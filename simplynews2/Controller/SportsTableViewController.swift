...
class SportsTableViewController: UITableViewController , XMLParserDelegate {
    ...
    func loadData() {
        url = URL(string: "http://feeds.sport24.co.za/articles/Sport/Featured/TopStories/rss")!
        loadRss(url);
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "openPagefour" {
            let indexPath: IndexPath = self.tableView.indexPathForSelectedRow!
            let selectedURL: String = (Feeds[indexPath.row] as AnyObject).object(forKey: "link") as! String
            
            
            let sportinstance: SportsWebViewController = segue.destination as! SportsWebViewController
            sportinstance.selectedcellurl = selectedURL as String
        }
    }
