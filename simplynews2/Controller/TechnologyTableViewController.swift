...
class TechnologyTableViewController: UITableViewController , XMLParserDelegate {
    ...
    func loadData() {
        url = URL(string: "https://www.techworld.com/news/rss")!
        loadRss(url);
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "openPagetwo" {
            let indexPath: IndexPath = self.tableView.indexPathForSelectedRow!
            let selectedURL: String = (Feeds[indexPath.row] as AnyObject).object(forKey: "link") as! String
            
            
            let techinstance: TechnologyWebViewController = segue.destination as! TechnologyWebViewController
            techinstance.selectedcellurl = selectedURL as String
        }
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "technologycell", for: indexPath)
